...
export const getAssociacoesPorCidade = (req, res) => {
    ...
    const sql = `SELECT associacao_id, associacao_nome_fantasia FROM associacoes WHERE associacao_cidade_id = ?`;
    db.query(sql, [id], (err, results) =>{
        if (err) {
            console.error("Erro ao buscar associações por cidade:", err);
            return res.status(500).json({ error: "Erro ao buscar associações por cidade" });
        }
        res.json(results);
    })
}
...
export const putAssociacao = (req, res) => {
    ...
    db.query(sql, [associacao_nome, associacao_nome_fantasia, associacao_cnpj, associacao_cidade_id, id], (err, results) => {
        if (err) {
